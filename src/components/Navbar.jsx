      ...
      window.scrollTo(0,0)}}>
       <LazyLoadImage
        ...
        <ul className='list-none phones:hidden sm:flex flex-row gap-10'>
         ...
         className={`${active === link.title ? "text-primary    underline" : "text-black"}  hover:text-black text-xs 2Xpc:text-sm font-light cursor-pointer`}
         onClick={() => {setActive(link.title)}}
         whileHover={{ scale: 1.1 }}
       >
         <a className='text-xs 2Xpc:text-sm' href={`#${link.id}`}> {link.title}</a>
       </motion.li>
          ))}
        </ul>
